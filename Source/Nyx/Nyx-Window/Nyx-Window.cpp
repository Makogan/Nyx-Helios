...
Nyx_Window::Nyx_Window(string name, void(*w_func)(), GLFWwindow* s_window, bool visible)
{
	...
	glfwWindowHint(GLFW_OPENGL_PROFILE, GLFW_OPENGL_ANY_PROFILE);//Use GLFW defaults
	glfwWindowHint(GLFW_DECORATED, GLFW_TRUE);//Make the window decorated
    glfwWindowHint(GLFW_OPENGL_DEBUG_CONTEXT, GLFW_TRUE);//Enable debugging messages
    ...
    //Verify that the window is correct
	if (!window)
	{
    ...
    window_name = name;
    //Initialize GLEW for current window
    init_glew();
    //clear window
    ...
    set_callback(framebuffer_size_callback);
    //Set the end of the log section frame
    record_log(string(80,'-'));
}
